...
const Graph: React.FC<GraphProps> = ({ title, dataKey, domain }) => {
  ...
  const [data, setData] = useState<({ date: Date; value: string } | null)[]>(
    []
  );
...
export const RelativeHumidityGraph: React.FC = () => {
  return (
    <Graph title="相対湿度" dataKey="relative_humidity" domain={[0, 50]} />
  );
};

export const SeismicIntensityGraph: React.FC = () => {
  return (
    <Graph title="地震強度" dataKey="seismic_intensity" domain={[0, 50]} />
  );
};
